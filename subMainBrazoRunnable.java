...
public class subMainBrazoRunnable{
	...
	private ArrayList<BrazoRunnable> Brazo = new ArrayList<>();
		...
		do {
			...
			switch (op) {
				...
				case 2:
					...
					break;
				case 3:
					System.out.println(ANSI_RED+"\tMODIFICAR ALMACEN"+ANSI_WHITE);
					mostrarAlmacen();
					modificarAlmacen();
	...
	public void setBrazos() {
		...
		do{
			...
			BrazoRunnable New_Brazo = new BrazoRunnable(index, Num_T, Almacen.get(id_contenedor - 1));
		...
		if (Almacen.get(opcion-1).getCapacidad() == 0) {
			...
			for (int i = 0; i < Brazo.size(); i++) {
				...
				if (Brazo.get(i).hilo.getIdContenedor() == opcion) {
					flag = true;
					Thread Hilo = new Thread(Brazo.get(i));
					Hilo.start();// hilos;
					...
					//	Hilo.join();
						...
						que está actualmente hasta que el hilo especificado esté muerto
						*/
